...
class HumanPlayer(Player):
    """A human hangman player."""

    def turn(self, state):
        """Print the board and prompt the player for a guess.
        ...
        """
        print(state)
        prompt = f"{self.name}, guess a letter or type a word to solve the "
        prompt += "puzzle: "
        return input(prompt)


class ComputerPlayer(Player):
    """A computer hangman player.

    Attributes:
        name (str): the computer player's name.
        vocab (list of str): words the computer can use while guessing.
    """

    def __init__(self, name, vocab):
        """Set the computer player's name and vocabulary.

        Args:
            name (str): the computer player's name.
            vocab (list of str): words the computer can use.
    ...
    def turn(self, state):
        """Choose a letter or a word based on the current board.

        Args:
            state (GameState): the current state of the game.

        Returns:
            str: a guessed letter or a guessed word.
        """
        candidates = self._candidate_words(state)

        if len(candidates) == 1:
            return candidates[0]

        letter = self._best_letter(candidates, state)

        if letter is not None:
            return letter

        return choice(sorted(state.unguessed))

    def _candidate_words(self, state):
        """Return vocab words that still fit the current board."""
        candidates = []

        for word in self.vocab:
            if word in state.guessed_words:
                continue
            if not state.match(word):
                continue
            if any(letter in word for letter in state.bad_guesses):
                continue
            candidates.append(word)

        return candidates

    def _best_letter(self, candidates, state):
        """Return the most useful unguessed letter from the candidate words."""
        if not candidates:
            return None
        ...
        best_count = max(letter_counts.values())
        best_letters = []
